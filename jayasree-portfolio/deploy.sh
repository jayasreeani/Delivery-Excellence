#!/usr/bin/env bash
# Deploy Jayasree Kuniyil portfolio to Vercel (requires VERCEL_TOKEN).
set -euo pipefail
...
fs.writeFileSync(outPath, JSON.stringify({ name: 'jayasree-kuniyil-portfolio', target, files: manifest }));
